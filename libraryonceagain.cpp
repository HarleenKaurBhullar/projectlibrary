...
class administrator : public Book, public studentdetails{             //Administartor class attribute to handle all administartion related 
    ...
    void addbook(){    
        if(!fh.is_open()){                                                //function to add book to binary file that stores books using
               fh.close(); 
               }                                            //previous enter function
        fh.open("booksagain.dat",ios::out|ios::app|ios::binary); 
        if(!fh.is_open()){
...
void adminfindbook(){
    ...
    string bookno;
    cin >> bookno;
    string fline = findbook(bookno);
    if (fline.empty()) {
        cout << "Book not found!" << endl;
        return;
    }
    cout << "Book details: " << fline << endl;
}
...
void deletebook() {                         //function to delete record of a book
    ...
    if (found) {
       if (remove("booksagain.dat") != 0) {
            perror("Error deleting original file");
            return;
        }

        // Attempt to rename the temporary file
        if (rename("temp.dat", "booksagain.dat") != 0) {
